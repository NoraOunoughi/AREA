...
export default function Weather() {
  ...
  useEffect(() => {
    ...
    const getActionReaction = async () => {
      ...
      if (docSnap.exists) {
        ...
        const allParams = await getAllParams(data, "spotify");
  ...
  const renderTitle = () => {
    if (title !== "") {
      return (
        <div
          className="spotifyArea-newAreaContent"
          style={{ color: "#50C878" }}
        >
          Title: {title}
        </div>
  ...
  const renderReactionParameter = () => {
    if (reaction !== "" && reaction.args.length === 3) {
      if (
        ...
        return (
          <span className="spotifyArea-span">
            <div
              className="spotifyArea-newAreaContent"
              style={{ color: "#50C878" }}
            >
      ...
      return (
        <form onSubmit={handleSubmitArea}>
          <input
            ...
            type="text"
            name="name"
            className="spotifyArea-searchButton"
          ...
          <input
            hidden={isForm}
            type="submit"
            className="spotifyArea-sendButton"
            value="Send"
          ></input>
        </form>
      );
  ...
  return (
    <div className="spotifyArea">
      <div className="spotifyArea-container">
        <div className="spotifyArea-AreaTest">
          <div className="spotifyArea-Area">
            <div className="spotifyArea-Title">
              Spotify Creator
            </div>
            <div>
              <input
                type="text"
                name="name"
                className="spotifyArea-titleButton"
                placeholder="Title..."
                value={title}
                onChange={(e) => setTitle(e.target.value)}
              />
            </div>
            <div className="spotifyArea-containerButton">
              <div className="spotifyArea-dropdown">
                <div className="spotifyArea-btn" onClick={hidenAction}>
                  Actions
                </div>
                <div className="spotifyArea-hidenAction" hidden={isAction}>
                  {isLoading ? (
                    <div>Loading...</div>
                  ) : (
                    allActionsPage.map((action, index) => (
                      <div className="spotifyArea-btnPattern">
                        <div
                          key={action.service + action.name}
                          className="spotifyArea-btnContent"
              ...
              <div className="spotifyArea-dropdown">
                <div className="spotifyArea-btn" onClick={hidenReaction}>
                  <div>Reactions</div>
                </div>
                <div className="spotifyArea-btnPattern" hidden={isReaction}>
                  {isLoading ? (
                    <div>Loading...</div>
                  ) : (
            ...
            <div className="spotifyArea-newArea">
              <div className="spotifyArea-newAreaContainer">
                {renderTitle()}
                {renderAction()}
                {renderActionParameter()}
                {renderReaction()}
                {renderReactionParameter()}
                <div className="spotifyArea-newAreaDelete" onClick={clearArea}>
                  clear
                </div>
              </div>
            </div>
