...
export default function Weather() {
  ...
  useEffect(() => {
    ...
    const getActionReaction = async () => {
      ...
      if (docSnap.exists) {
        ...
        const allParams = await getAllParams(data, "youtube");
  ...
  const renderTitle = () => {
    if (title !== "") {
      return (
        <div
          className="youtubeArea-newAreaContent"
          style={{ color: "#50C878" }}
        >
          Title: {title}
        </div>
  ...
  const renderReactionParameter = () => {
    if (reaction !== "" && reaction.args.length === 3) {
      if (
        ...
        return (
          <span className="youtubeArea-span">
            <div
              className="youtubeArea-newAreaContent"
              style={{ color: "#50C878" }}
            >
      ...
      return (
        <form onSubmit={handleSubmitArea}>
          <input
            ...
            type="text"
            name="name"
            className="youtubeArea-searchButton"
          ...
          <input
            hidden={isForm}
            type="submit"
            className="youtubeArea-sendButton"
            value="Send"
          ></input>
        </form>
      );
  ...
  return (
    <div className="youtubeArea">
      <div className="youtubeArea-container">
        <div className="youtubeArea-AreaTest">
          <div className="youtubeArea-Area">
            <div className="youtubeArea-Title">
              Youtube Creator
            </div>
            <div>
              <input
                type="text"
                name="name"
                className="youtubeArea-titleButton"
                placeholder="Title..."
                value={title}
                onChange={(e) => setTitle(e.target.value)}
              />
            </div>
            <div className="youtubeArea-containerButton">
              <div className="youtubeArea-dropdown">
                <div className="youtubeArea-btn" onClick={hidenAction}>
                  Actions
                </div>
                <div className="youtubeArea-hidenAction" hidden={isAction}>
                  {isLoading ? (
                    <div>Loading...</div>
                  ) : (
                    allActionsPage.map((action, index) => (
                      <div className="youtubeArea-btnPattern">
                        <div
                          key={action.service + action.name}
                          className="youtubeArea-btnContent"
              ...
              <div className="youtubeArea-dropdown">
                <div className="youtubeArea-btn" onClick={hidenReaction}>
                  <div>Reactions</div>
                </div>
                <div className="youtubeArea-btnPattern" hidden={isReaction}>
                  {isLoading ? (
                    <div>Loading...</div>
                  ) : (
            ...
            <div className="youtubeArea-newArea">
              <div className="youtubeArea-newAreaContainer">
                {renderTitle()}
                {renderAction()}
                {renderActionParameter()}
                {renderReaction()}
                {renderReactionParameter()}
                <div className="youtubeArea-newAreaDelete" onClick={clearArea}>
                  clear
                </div>
              </div>
            </div>
