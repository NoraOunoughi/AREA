...
export default function Weather() {
  ...
  useEffect(() => {
    ...
    const getActionReaction = async () => {
      ...
      if (docSnap.exists) {
        ...
        const allParams = await getAllParams(data, "github");
  ...
  const renderTitle = () => {
    if (title !== "") {
      return (
        <div
          className="githubArea-newAreaContent"
          style={{ color: "#50C878" }}
        >
          Title: {title}
        </div>
  ...
  const renderReactionParameter = () => {
    if (reaction !== "" && reaction.args.length === 3) {
      if (
        ...
        return (
          <span className="githubArea-span">
            <div
              className="githubArea-newAreaContent"
              style={{ color: "#50C878" }}
            >
      ...
      return (
        <form onSubmit={handleSubmitArea}>
          ...
          <input
            hidden={isForm}
            type="submit"
            className="githubArea-sendButton"
            value="Send"
          ></input>
        </form>
      );
  ...
  return (
    <div className="githubArea">
      <div className="githubArea-container">
        <div className="githubArea-AreaTest">
          <div className="githubArea-Area">
          <div className="githubArea-Title">
              Github Creator
            </div>
            <div>
              <input
                type="text"
                name="name"
                className="githubArea-titleButton"
                placeholder="Title..."
                value={title}
                onChange={(e) => setTitle(e.target.value)}
              />
            </div>
            <div className="githubArea-containerButton">
              <div className="githubArea-dropdown">
                <div className="githubArea-btn" onClick={hidenAction}>
                  Actions
                </div>
                <div className="githubArea-hidenAction" hidden={isAction}>
                  {isLoading ? (
                    <div>Loading...</div>
                  ) : (
                    allActionsPage.map((action, index) => (
                      <div className="githubArea-btnPattern">
                        <div
                          key={action.service + action.name}
                          className="githubArea-btnContent"
              ...
              <div className="githubArea-dropdown">
                <div className="githubArea-btn" onClick={hidenReaction}>
                  <div>Reactions</div>
                </div>
                <div className="githubArea-btnPattern" hidden={isReaction}>
                  {isLoading ? (
                    <div>Loading...</div>
                  ) : (
            ...
            <div className="githubArea-newArea">
              <div className="githubArea-newAreaContainer">
                {renderTitle()}
                {renderAction()}
                {renderActionParameter()}
                {renderReaction()}
                {renderReactionParameter()}
                <div className="githubArea-newAreaDelete" onClick={clearArea}>
                  clear
                </div>
              </div>
            </div>
