...
export default function Weather() {
  ...
  useEffect(() => {
    ...
    const getActionReaction = async () => {
      ...
      if (docSnap.exists) {
        ...
        const allParams = await getAllParams(data, "gmail");
  ...
  const renderTitle = () => {
    if (title !== "") {
      return (
        <div
          className="gmailArea-newAreaContent"
          style={{ color: "#50C878" }}
        >
          Title: {title}
        </div>
      ...
      return (
        <form onSubmit={handleSubmitArea}>
          <input
            ...
            type="text"
            name="name"
            className="gmailArea-searchButton"
          ...
          <input
            hidden={isForm}
            type="submit"
            className="gmailArea-sendButton"
            value="Send"
          ></input>
        </form>
      );
  ...
  return (
    <div className="gmailArea">
      <div className="gmailArea-container">
        <div className="gmailArea-AreaTest">
          <div className="gmailArea-Area">
            <div className="gmailArea-Title">
              Gmail Creator
            </div>
            <div>
              <input
                type="text"
                name="name"
                className="gmailArea-titleButton"
                placeholder="Title..."
                value={title}
                onChange={(e) => setTitle(e.target.value)}
              />
            </div>
            <div className="gmailArea-containerButton">
              <div className="gmailArea-dropdown">
                <div className="gmailArea-btn" onClick={hidenAction}>
                  Actions
                </div>
                <div className="gmailArea-hidenAction" hidden={isAction}>
                  {isLoading ? (
                    <div>Loading...</div>
                  ) : (
                    allActionsPage.map((action, index) => (
                      <div className="gmailArea-btnPattern">
                        <div
                          key={action.service + action.name}
                          className="gmailArea-btnContent"
              ...
              <div className="gmailArea-dropdown">
                <div className="gmailArea-btn" onClick={hidenReaction}>
                  <div>Reactions</div>
                </div>
                <div className="gmailArea-btnPattern" hidden={isReaction}>
                  {isLoading ? (
                    <div>Loading...</div>
                  ) : (
            ...
            <div className="gmailArea-newArea">
              <div className="gmailArea-newAreaContainer">
                {renderTitle()}
                {renderAction()}
                {renderActionParameter()}
                {renderReaction()}
                {renderReactionParameter()}
                <div className="gmailArea-newAreaDelete" onClick={clearArea}>
                  clear
                </div>
              </div>
            </div>
